...
adapt(val) = val * dattorroSr / (mysr);

// slider

//guisr = vslider("Sample Rate[style:radio{'44100':0;'48000':1;'96000':2}]",0, 0, 2, 1);
//mysr = select3(guisr, 44100, 48000, 96000);
mysr = 48000;

pdly = vslider("[0]preDelay", 0, 0, mysr, 1); //si può impostare SR
decay = vslider("[1]decay", 0.7, 0, 1, 0.01);
...
damp = 1-vslider("[5]damping", 0.7, 0, 1, 0.01);
...
exui = exgroup(vslider("Freq", 16, 0, 20, 0.001));

dattorroGroup = hgroup("Dattorro Reverb", dattorro);

lp1p(a) = _*(a) : +~*(1-a);

apf(idif, t) = (ma.sub : (*(idif) <: de.delay(mysr, t), _))~*(idif) : +;  
                //ma.sub inverte l'ordine degli operatori
//process = ba.pulsen(1, ma.SR) : apf;

apfm(idif, t, ex) = (+ : (*(idif) <: de.sdelay(mysr, 512, t+(oscS*ex)), _))~*(idif) : -; //esternare oscillatore


yL =  apfm(ddiff1, adapt(672), exui) : de.delay(mysr, adapt(4453)) : lp1p(damp) : *(decay) : apf(ddiff1, adapt(1800)) : de.delay(ma.SR, adapt(3720)) : *(decay);
yR =  apfm(ddiff1, adapt(908), exui) : de.delay(mysr, adapt(4217)) : lp1p(damp) : *(decay) : apf(ddiff2, adapt(2656)) : de.delay(ma.SR, adapt(3163)) : *(decay);
...
dattorro = _ : de.delay(mysr, pdly) : lp1p(bw) : apf(idif1, adapt(142)) : apf(idif1, adapt(107)) : apf(idif2, adapt(379)) : apf(idif2, adapt(277)) <: tank;   //modifica decay dopo l'uscita

//process = fi.allpassnt(1, 1);    //filtro allpass con topologia lattice
//process = _ <: _*(0.5),dattorro :>/(2);

process = dattorroGroup;